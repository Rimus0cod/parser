"""
email_sender.py — HTML email notification for the imoti.bg rental scraper.
...
def build_plain_text(ads: list[dict[str, Any]], today: str) -> str:
    """
    Fallback plain-text version of the email body.
...
def send_email(
    ...
    """
    Send an HTML email summarising newly found apartment listings.

    The function is a no-op when:
    • config.dry_run is True
    • config.email_to is empty
    • The ads list is empty

    Args:
        config: Populated Config object (must have email_* and smtp_* fields).
        ads:    List of ad dicts to include in the email.
        today:  Date string for the subject line (defaults to today's date).
